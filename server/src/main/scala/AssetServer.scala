...
object AssetServer extends App {
  ...
  implicit def myRejectionHandler: RejectionHandler = {
    RejectionHandler.newBuilder()
      ...
      .handleAll[Rejection] {
      _ => complete(MethodNotAllowed, s"Can't do that! Supported!")
    }
      .handleNotFound {
  ...
  private val fileHandler = handleExceptions(ExceptionHandler {
    case _: NoSuchFileException =>
      complete(HttpResponse(status = StatusCodes.InternalServerError, entity = HttpEntity(ContentTypes.`text/plain(UTF-8)`, "File Not Found")))
  })
  ...
  private val javascriptRoute: Route = path("js" / Segment / Segment) { (module, path) =>
    encodeResponse {
      val resourcePath = s"$static_base/apps/$module/$path"
      fileHandler {
        complete(HttpEntity(MediaTypes.`application/javascript` withCharset HttpCharsets.`UTF-8`, Files.readAllBytes(getPath(resourcePath))))
      }
    }
  }

  private val cssRoute: Route = path("css" / Segment / Segment) { (module, path) =>
    encodeResponse {
      val resourcePath = s"$static_base/apps/$module/$path"
      fileHandler {
        complete(HttpEntity(MediaTypes.`text/css` withCharset HttpCharsets.`UTF-8`, Files.readAllBytes(getPath(resourcePath))))
      }
    }
  }
    ...
    pathPrefix(Segment) { module =>
      path(Segments) { absUrl =>
        pathEndOrSingleSlash {
          ...
          complete(HttpEntity(ContentTypes.`text/html(UTF-8)`, Files.readAllBytes(getPath(resourcePath))))
        }
      }
    }
  }
  ...
  val bindingHttp = Http().bindAndHandle(route, host, httpPort)
